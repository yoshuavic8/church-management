...
export const AdminMemberScanner: React.FC<AdminMemberScannerProps> = ({
  ...
  // Check admin permissions - only after auth is checked
  const isAdmin = authChecked && user && (user.role === 'admin' || (user.role_level && user.role_level >= 4));

  // Don't show access denied until we've confirmed user is loaded
  if (!authChecked || !user) {
    return (
  ...
  if (meeting && !meeting.live_checkin_active) {
    return (
      <div className={`p-4 bg-yellow-50 border border-yellow-200 rounded-lg text-center ${className}`}>
        ...
        <p className="text-yellow-600 text-sm mt-1">
          Please enable live attendance for this meeting first
        </p>
      </div>
    );
  }
